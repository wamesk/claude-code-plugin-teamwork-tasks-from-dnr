...
OUT = "/Users/sucho/WAME/Laravel/wame-reality/docs/DNR_Strecnianska_TeamworkTasks.xlsx"
OUT_MD = "/Users/sucho/WAME/Laravel/wame-reality/docs/DNR_Strecnianska_TeamworkTasks.md"

HEADER = ["TASKLIST", "TASK", "DESCRIPTION", "ASSIGN TO", "START DATE", "DUE DATE",
          "PRIORITY", "ESTIMATED TIME", "TAGS", "STATUS"]

# -----------------------------------------------------------------------------
# DNR passages (faithful prose copy of sections 4.1, 4.2, 4.3 from v1.2)
# -----------------------------------------------------------------------------

DNR_EXTENSION_1 = """ROZŠÍRENIE Č. 1 — ROČNÝ ZÁLOHOVÝ PREDPIS (DNR sekcia 4.1)
...
DNR_EXTENSION_2 = """ROZŠÍRENIE Č. 2 — AUTOMATICKÉ PÁROVANIE PLATIEB Z BANKY (DNR sekcia 4.2)
...
DNR_EXTENSION_3 = """ROZŠÍRENIE Č. 3 — ROČNÉ ZÚČTOVANIE SLUŽIEB (DNR sekcia 4.3)
...
# -----------------------------------------------------------------------------
# Tasks: business-friendly names, "Acceptance criteria" first then tech plan.
# Tags removed per request.
# -----------------------------------------------------------------------------


def task(name, minutes, priority, goal, acceptance, technical):
    """Build a task tuple: (name, est_minutes, priority, description).

    Description format:
        ## Akceptačné kritériá
        - [ ] item 1
        - [ ] item 2
        ### Závislosť (optional, kept as plain bullets)

        ---

        ## Cieľ
        <goal — 1-3 sentence framing for Claude Code>

        ## Technický popis
        <technical>

    Acceptance text uses normal `- ` bullets; we convert them to `- [ ]`
    checkbox bullets, except inside a `### Závislosť` / `### Závislosti` block.
    """
    out_lines = []
    in_dependency_block = False
    for raw in acceptance.rstrip().split("\n"):
        line = raw
        stripped = line.strip()
        if stripped.startswith("### "):
            label = stripped.lower()
            in_dependency_block = "závislos" in label or "dependency" in label or "dependencies" in label
    ...
    return (name, str(minutes), priority, desc)


TASKLISTS = [
    {
        "name": "Rozšírenie #1 — Ročný zálohový predpis (DNR 4.1)",
        "description": DNR_EXTENSION_1,
        "tasks": [
            task(
...
- `payment_date` na `invoice_payments` odlišuje dátum pripísania od `created_at` (potrebné pre bmail import).""",
            ...
            task(
                "Evidencia mesačných splátok v rámci predpisu",
                90, "High",
...
- Pest `InvoiceScheduleInstallmentTest::test_status_changes_with_payments` overí prechod stavov.""",
            ...
            task(
                "Logika výpočtu ročného zálohového predpisu",
                180, "High",
...
- Viacero rovnakých priestorov → zoskupiť per `premise_id` + `title`.""",
            ...
            task(
                'Funkcia „Pripraviť ročný zálohový predpis" v detaile zmluvy',
                120, "High",
...
- Ak ostane čas — vlastný ResourceTool `nova-components/AnnualAdvancePreview` so side-by-side náhľadom pred submit.""",
            ...
            task(
                "PDF výstup ročného zálohového predpisu (vzor SOMPET)",
                180, "High",
...
`public/css/invoice.css` — pridať `.installments` (border, padding) a `.premise-header` (light gray bg, bold).""",
            ...
            task(
                "Hromadné pripravenie zálohových predpisov pre všetky aktívne zmluvy",
                120, "Medium",
...
`Contract::actions()` — pridať `PrepareAnnualAdvanceBulk`.""",
            ...
            task(
                "Odoslanie zálohového predpisu nájomcovi e-mailom",
                120, "Medium",
...
`app/Nova/Invoice/ButtonsCard.php` — tlačidlo „Odoslať e-mailom" musí byť dostupné aj pre `type = '4'`.""",
            ...
            task(
                "Číselný rad ZP a evidencia variabilných symbolov zmlúv",
                60, "High",
...
- Soft-deleted zmluvy: VS sa môže reusnúť.""",
            ...
            task(
                "Testovanie a overenie ročného zálohového predpisu",
                210, "Medium",
...
≥ 80 % na nové service triedy.""",
    ...
    {
        "name": "Rozšírenie #2 — Automatické párovanie platieb z banky (DNR 4.2)",
        "description": DNR_EXTENSION_2,
        "tasks": [
            task(
                "Evidencia notifikácií z banky v systéme",
                60, "High",
...
- Index `['status', 'created_at']`, index `'parsed_variable_symbol'`.""",
            ...
            task(
                "Prehľad bankových notifikácií a manuálny zásah",
                90, "High",
...
Pridať do `NovaServiceProvider::mainMenu()` položku „Bmail importy".""",
            ...
            task(
                "Pripojenie systému na bankovú schránku",
                60, "High",
...
- Ops vytvorí schránku `bmail-strecnianska@reality.wame.sk` a heslo doplní do `.env`.""",
            ...
            task(
                "Rozpoznanie obsahu notifikácie z Tatra banky",
                180, "High",
...
- Platba presahujúca všetky splátky: posledná = `overpay`, zvyšok poznámka, manuálne.""",
            ...
            task(
                "Denné automatické sťahovanie platieb",
                60, "High",
...
- Overiť že `crontab -e` na produkcii má `* * * * * cd /path && php artisan schedule:run >> /dev/null 2>&1`.""",
            ...
            task(
                "Manuálna úprava a rozdelenie platieb",
                60, "Medium",
...
- Akcia `AssignBmailToContract` z Task 2.2 už pokrýva manuálne pridelenie.""",
            ...
            task(
                "Testovanie a overenie párovania platieb",
                30, "Medium",
...
```""",
    ...
    {
        "name": "Rozšírenie #3 — Ročné zúčtovanie služieb (DNR 4.3)",
        "description": DNR_EXTENSION_3,
        "tasks": [
            task(
                "Evidencia ročného zúčtovania služieb v systéme",
                120, "High",
...
- Index `[annual_settlement_id, contract_id, service_type]`.""",
            ...
            task(
                "Štruktúra údajov o vyúčtovaní per zmluva a priestor",
                60, "High",
...
- `app/Enums/SettlementStatus.php` (draft/approved/sent).""",
            ...
            task(
                "Výpočtový engine ročného zúčtovania (kľúčový bod)",
                300, "High",
...
- Priestor správcu bez zmluvy: m² v deliteľi, ale `final_cost` ide na firmu (samostatný riadok bez contract_id alebo pole `building_overhead` v metadata).""",
            ...
            task(
                "Rozhranie pre prípravu, kontrolu a schválenie zúčtovania",
                120, "High",
...
- Po `approved`: úprava `final_cost` zakázaná (Policy `update`), zostáva otvorená iba akcia „Regenerovať jedno PDF" (Task 3.7).""",
            ...
            task(
                "PDF výstup ročného zúčtovania pre nájomcu",
                180, "High",
...
`public/css/settlement.css` (analógia k `invoice.css`).""",
            ...
            task(
                "Hromadné odoslanie ročných zúčtovaní e-mailom",
                60, "Medium",
...
- Error handling per zmluva (log + zhrnutie v ActionResponse).""",
            ...
            task(
                "Audit log úprav a oprava jednotlivých zúčtovaní",
                60, "Low",
...
- Otvorené aj pre status = `sent` (DNR 4.3.4 bod 8).""",
            ...
            task(
                "Testovanie a overenie ročného zúčtovania",
                60, "Medium",
...
def build_md():
    """Generate the companion Markdown plan with identical content as XLSX."""
    lines = []
    lines.append("# DNR Strečnianska v1.2 — Teamwork Task Listy")
    lines.append("")
    lines.append("Zdroj: `DNR_Strecnianska_v1.2.docx` (sekcia 4.1 – 4.3)")
    lines.append("Projekt: Družstvo lekárov — Strečnianska, rozšírenie Realitného systému WAME")
    lines.append("Cenová kalkulácia: PON1107 (6 MD back-end)")
    lines.append("")
    lines.append("Súbor `DNR_Strecnianska_TeamworkTasks.xlsx` v tom istom adresári je 1:1 import-ready pre Teamwork (Options → Import → Microsoft Excel).")
    lines.append("")
    lines.append("## Konvencie")
    lines.append("")
    lines.append("- **Cesty súborov** sú absolútne k repo rootu `/Users/sucho/WAME/Laravel/wame-reality`.")
    lines.append("- **Modulárna konvencia**: projekt nie je modulárny (flat `app/Models`, `app/Nova`).")
    lines.append("- **DB**: ULID PK (`$table->ulid('id')->primary()`), `softDeletes`, `timestampsTz`. FK cez `foreignIdFor(Class)`.")
    lines.append("- **Code style**: po každom logickom celku spusti `./vendor/bin/pint`. Komenty/PHPDoc v angličtine.")
    lines.append("- **Existujúce enumy**: `InvoiceType`, `InvoiceStatus`, `BillingDay`, `RecurrenceCycle`, `DynamicQuantity`, `ContractItemCustomPrice` v `app/Enums`.")
    lines.append("- **Testovanie**: Pest, DB `reality_testing`.")
    lines.append("- **Commity**: format `TYPE(scope): Message` (CREATE/UPDATE/FIX/EDIT…); bez Co-Authored-By.")
    lines.append("")
    lines.append("## Mapovanie pracnosti")
    lines.append("")
    lines.append("| Task list | DNR | Tasky | MD | Hodín |")
    lines.append("|---|---|---|---|---|")
    total_minutes = 0
    for i, tl in enumerate(TASKLISTS, start=1):
        tl_minutes = sum(int(t[1]) for t in tl["tasks"])
        total_minutes += tl_minutes
        md_value = tl_minutes / 60 / 8
        lines.append(f"| {i}. {tl['name'].split('—', 1)[1].strip()} | 4.{i} | {len(tl['tasks'])} | {md_value:.1f} | {tl_minutes/60:.1f} |")
    lines.append(f"| **Spolu** | | **{sum(len(tl['tasks']) for tl in TASKLISTS)}** | **{total_minutes/60/8:.1f}** | **{total_minutes/60:.1f}** |")
    lines.append("")

    for i, tl in enumerate(TASKLISTS, start=1):
        lines.append("---")
        lines.append("")
        lines.append(f"## TASK LIST {i}: {tl['name']}")
        lines.append("")
        lines.append("### Pôvodné znenie DNR")
        lines.append("")
        for paragraph in tl["description"].strip().split("\n"):
            lines.append("> " + paragraph if paragraph else ">")
        lines.append("")

        for j, t in enumerate(tl["tasks"], start=1):
            name, est_min, priority, desc = t
            hours = int(est_min) / 60
            lines.append(f"### Task {i}.{j} — {name}")
            lines.append("")
            lines.append(f"**Priorita:** {priority} · **Odhad:** {hours:g}h ({est_min} min)")
            lines.append("")
            # Description už obsahuje "## Akceptačné kritériá ... --- ... ## Technický popis"
            # Pre MD vykreslíme jej obsah s podtitulkami a separátorom.
            lines.append(desc)
            lines.append("")

    lines.append("---")
    lines.append("")
    lines.append("## Globálne výstupy projektu (Definition of Done)")
    lines.append("")
    lines.append("- [ ] `php artisan migrate:fresh --seed` v `wame_reality_fix` prejde.")
    lines.append("- [ ] `./vendor/bin/pest` — všetky existujúce + nové testy zelené.")
    lines.append("- [ ] `./vendor/bin/pint --test` — bez problémov.")
    lines.append("- [ ] Cron `bmail:import` v `php artisan schedule:list`.")
    lines.append("- [ ] DNR sekcia 10 — používateľská príručka pre admina v `docs/uzivatelska_prirucka.md` (krátka, 2–3 strany, so screenshotmi z Nova).")
    lines.append("- [ ] Online zaškolenie 1–2h (DNR 10.1).")
    lines.append("- [ ] DNR sekcia 8 podklady doplnené: `docs/podklady/SOMPET_predpis.pdf`, `tatra_credit_sample.txt`, `vyuctovanie_predchadzajuci_rok.pdf`.")
    lines.append("")
    lines.append("## Otázky/závislosti pred štartom")
    lines.append("")
    lines.append("1. **SOMPET PDF vzor** — overiť, že je v repo `docs/podklady/`, ak nie, vyžiadať od klienta pred Task 1.5.")
    lines.append('2. **Reálny vzor Tatra banka notifikácie** — pred Task 2.4 (DNR sekcia 8 bod 2 „prisľúbené").')
    lines.append('3. **Workshop k algoritmu zúčtovania** — pred Task 3.3 (DNR „Dospresnenie algoritmu v príprave").')
    lines.append("4. **Mapping `ContractTemplateItem` → `SettlementServiceType`** — chýbajúca konvencia. Pravdepodobne pridať enum stĺpec `service_key` na `contract_template_items`.")
    lines.append("5. **IMAP credentials** — vytvoriť dedikovanú schránku `bmail-strecnianska@reality.wame.sk`, ops doplní do `.env`.")
    lines.append("6. **Spatie Activity Log** — overiť, či projekt už má `spatie/laravel-activitylog`. Ak áno, použiť ho v Task 3.7.")
    lines.append("")
...
def main():
    ...
    tasklist_fill = PatternFill("solid", fgColor="D9E2F3")
    tasklist_font = Font(bold=True, size=11)
    wrap_top = Alignment(wrap_text=True, vertical="top")

    row = 2
    for tl in TASKLISTS:
        ws.cell(row=row, column=1, value=tl["name"]).font = tasklist_font
        ws.cell(row=row, column=1).fill = tasklist_fill
        ws.cell(row=row, column=1).alignment = wrap_top
        ws.cell(row=row, column=3, value=tl["description"])
        ws.cell(row=row, column=3).fill = tasklist_fill
        ws.cell(row=row, column=3).alignment = wrap_top
        ws.cell(row=row, column=10, value="Active").fill = tasklist_fill
        for col_idx in range(1, 11):
            ws.cell(row=row, column=col_idx).border = border
            if col_idx not in (1, 3, 10):
                ws.cell(row=row, column=col_idx).fill = tasklist_fill
        row += 1

        for t in tl["tasks"]:
            name, est_min, priority, desc = t
            ws.cell(row=row, column=2, value=name).alignment = wrap_top
            ws.cell(row=row, column=3, value=desc).alignment = wrap_top
            ws.cell(row=row, column=7, value=priority)
            ws.cell(row=row, column=8, value=int(est_min))
            # column 9 (TAGS) left empty per request
            ws.cell(row=row, column=10, value="Active")
            for col_idx in range(1, 11):
                ws.cell(row=row, column=col_idx).border = border
            row += 1

    widths = {1: 40, 2: 60, 3: 110, 4: 20, 5: 14, 6: 14, 7: 10, 8: 14, 9: 14, 10: 14}
    ...
    wb.save(OUT)
    print(f"Saved: {OUT}")
    print(f"Tasklists: {len(TASKLISTS)}")
    print(f"Tasks total: {sum(len(tl['tasks']) for tl in TASKLISTS)}")
    print(f"Estimated minutes total: {sum(int(t[1]) for tl in TASKLISTS for t in tl['tasks'])} ({sum(int(t[1]) for tl in TASKLISTS for t in tl['tasks']) / 60:.1f} h)")
